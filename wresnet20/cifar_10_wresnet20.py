...
def conv_block(M, filters, kernel_size, stage, block, weight_decay):
    ...
    #Main path
    M = Conv2D(filters, (kernel_size, kernel_size), activation='relu', kernel_initializer='he_uniform', \
            kernel_regularizer=l2(weight_decay), padding='valid', name=conv_name+'a', strides=(2,2))(M)
    M = BatchNormalization(axis=3, name=bn_name+'a')(M)
    M = Dropout(0.3)(M)
...
def WResNet20(num_classifiers, input_shape, padding):
    '''
    ...
    M = Conv2D(16*8, (3, 3), activation='relu', kernel_initializer='he_uniform', \
            kernel_regularizer=l2(weight_decay), input_shape=input_shape, name='conv_s1_b1', padding='same')(M_shape)
    M = BatchNormalization(axis=3, name='batchN_s1_b1')(M)

    #Stage 2 2D-Convolution
    M = identity_block(M, 16*8, 3, 2, 1, weight_decay)
    M = identity_block(M, 16*8, 3, 2, 2, weight_decay)
    M = identity_block(M, 16*8, 3, 2, 3, weight_decay)
    M = ZeroPadding2D(padding=1)(M)

    #Stage 3 2D-Convolution
    M = conv_block(M, 32*8, 3, 3, 1, weight_decay)
    M = identity_block(M, 32*8, 3, 3, 2, weight_decay)
    M = identity_block(M, 32*8, 3, 3, 3, weight_decay)
    M = ZeroPadding2D(padding=1)(M)


    #Stage 4 2D-Convolution
    M = conv_block(M, 64*8, 3, 4, 1, weight_decay)
    M = identity_block(M, 64*8, 3, 4, 2, weight_decay)
    M = identity_block(M, 64*8, 3, 4, 3, weight_decay)
    
    
    #AVGPOOL Layer
    M = AveragePooling2D((8,8), name="avg_pool")(M)
    ...
    model = Model(inputs=M_shape, outputs=M, name='WResNet20')
...
def main():
    ...
    trained_model = WResNet20(10, (32, 32, 3), 0)
